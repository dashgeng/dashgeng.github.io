...
#### 断言
* 在代码中触发一个断言(assertion)，将会结束代码运行并通过调试来找到值缺失的原因。
* 断言会在运行时判断一个逻辑条件的值，如果条件判断为 true，代码会继续运行；如果条件判断为 false，代码运行终止。
* 可以使用全局函数`assert`来写一个断言。向 assert 传入一个逻辑表达式，以及一条信息。当表达式为 false 时，这条信息会被显示。断言信息可以被省略。
	...
	```
* 断言适用情景：
 * 整数类型的下标索引被传入一个自定义下标脚本实现，但是下标索引值可能越界。
 * 需要给函数传入一个值，但非法值可能导致函数不能正常执行。
 * 一个可选值现在是 nil，但后面的代码运行需要一个非 nil 值。
